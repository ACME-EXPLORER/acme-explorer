  ...
  return (
    <>
      ...
      <div className='py-3 row'>
        ...
        <div className='col-2'>
          <Button onClick={onSubmit} color='primary' className='btn-md'><span><i className='fa fa-search' /> </span>Buscar</Button>
          {!selectedFinder.id && (state.model.keyword || state.model.minPrice || state.model.maxPrice)
            ? <a style={{ cursor: 'pointer' }} onClick={(e) => toggleSweetAlert({ e, setState, value: true })}><i className='mx-2 fa fa-save' /></a>
            : <div />}
        </div>
      </div>
    </>
