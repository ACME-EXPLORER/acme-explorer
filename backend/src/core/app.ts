...
class App {
  ...
  async _startServer() {
    ...
    this.logger.info(
      `Service ${this.env.NAME} started successfully on port ${this.env.PORT}`,
    );
  }
...
class RouteManager {
  ...
  getAuthMiddleware({ access, roles }) {
    return async (req, res, next) => {
      ...
      if (
        (access === 'public' || typeof access === 'object') &&
        method in access &&
  ...
  _loadRouteMethods(router, view) {
    ...
    Object.entries(view.methods).forEach(([$key, $value]) =>
      router[$key](path, access, $value),
    );
  }
}
